...
import os
from lib.model import data_loader, generator, SRGAN, test_data_loader, inference_data_loader, save_images, unet
from lib.ops import *
import math
import time
import numpy as np
...
Flags.DEFINE_string('pre_trained_model_type', 'unet', 'The type of pretrained model (SRGAN or unet)')
Flags.DEFINE_boolean('is_training', True, 'Training => True, Testing => False')
Flags.DEFINE_string('task', None, 'The task: SRGAN, unet')
# The data preparing operation
Flags.DEFINE_integer('batch_size', 16, 'Batch size of the input batch')
Flags.DEFINE_boolean('interpolation', True, 'Whether perform the interpolation')
...
if FLAGS.mode == 'test':
    ...
    # rewrite SRGAN: by Yucheng @ 03/06
    #FLAGS.flip = False
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(max_iter):
            print([test_data.inputs[i]].shape)
...
# The training mode
    ...
    elif FLAGS.task =='unet':
        Net = unet(data.inputs, data.targets, FLAGS)
    else:
    ...
    # Here if we restore the weight from the unet the var_list2 do not need to contain the discriminator weights
    ...
    if FLAGS.task == 'SRGAN':
        ...
        elif FLAGS.pre_trained_model_type == 'unet':
            var_list2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='generator')
        else:
    ...
    if not FLAGS.perceptual_mode == 'MSE':
        raise ValueError('no vgg anymore!!')
